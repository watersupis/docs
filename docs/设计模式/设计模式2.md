...
参考：http://design-patterns.readthedocs.org/zh_CN/latest/index.html

## 二、设计模式的七大原则

### 2.1 单一职责原则（SRP）

> **单一职责（Single Responsibility Principle）的特点**
...
这个原则的意思是：**每个接口中不存在子类用不到却必须实现的方法**，如果不然，就要将接口拆分。**使用多个隔离的接口，比使用单个接口（多个接口方法集成到一个的接口）要好**。

### 2.4 依赖倒置原则（DIP）

> **依赖倒置（Dependency Inversion Principle）的特点**

官方是这样解释的：**抽象不应该依赖具体，具体依赖于抽象**。换做通俗的话来说就是**面向接口**编程。

**依赖倒置原则：**

1. 高层模块（调用）不应该依赖于低层模块（实现）。两个都要依赖于抽象模块。
2. 抽象对象不应该依赖于具体对象，具体对象依赖于抽象对象。


​       简单来说，就是你高层的代码，不应该依赖于底层的代码。应该依赖抽象接口或者类。不管需求怎么改。我们只需要修改具体的实现细节就行了。

### 2.5 里氏代换原则（LSP）

> **里式代换（Liskov's Substitution Principle）的特点**

官方是这样解释的：**子类型必须能够完全替代父类型**。这是继承的原则简单来说就是父类的东西子类必须有，软件设计或代码开发中，子类继承父类时不应改变父类的行为/功能。
...
> **合成复用（Composite Reuse Principle）的特点**
...
public class A {
    public void Method1() {
    	System.out.print("我是方法一");
    }
    public void Method2() {
    	System.out.print("我是方法二");
    }
}
//依赖
public class B {
    public void Mthod1(A a) {
        System.out.print("调用A方法");
    }
}
...
![适配器类图.png](others/%E8%AE%BE%E8%AE%A1%E6%A8%A1%E5%BC%8F.assets/3GUMNPAEV884B3C8FD0ET2QOSL.png)
...
![image-20200705114257849](others/%E8%AE%BE%E8%AE%A1%E6%A8%A1%E5%BC%8F.assets/image-20200705114257849.png)
